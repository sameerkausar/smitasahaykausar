...
Searching her name should return this site. Most of that is not markup.

What the repo already does, all regenerated by `tools/unbundle.py`:

- a `<title>`, description, canonical URL and Open Graph tags
- a schema.org **Person** block in the head, naming her ORCID, Google Scholar
  and LinkedIn under `sameAs`, read out of the contact section so it cannot
  drift from what the page links to
- `sitemap.xml`, with `lastmod` stamped on every rebuild
- `robots.txt` pointing at the sitemap

What still has to be done by hand, and matters more:

1. **Google Search Console.** Add `smitasahaykausar.com` at
   [search.google.com/search-console](https://search.google.com/search-console),
   verify by DNS TXT record at Squarespace, submit `sitemap.xml`, then use
   **URL Inspection → Request indexing** on the homepage. Without this, a new
   domain can sit unindexed for weeks. With it, usually days.

2. **Make the profiles point back.** `sameAs` is a claim; it only counts when
   the other end agrees. Put `https://smitasahaykausar.com` in the website
   field of her LinkedIn profile, her ORCID record, and her Google Scholar
   profile. Three links from sites Google already trusts, all confirming the
   same person, is the strongest single signal available here.

3. **Ask for the university profile.** A link from a
   `utoledo.edu` page — a lab roster, a student directory, a department
   listing — outweighs almost anything else on this list. Her lab or program
   coordinator can usually add it.

4. **Anywhere her name already appears.** The Toledo Blade byline, the Way To
   Goal article, conference programmes, a ResearchGate profile. Each is a
   chance to link the domain.

Ranking for a personal name is an identity problem before it is a keyword
one: Google has to be convinced that this domain and that person are the same
entity. The markup states it; the backlinks prove it.
